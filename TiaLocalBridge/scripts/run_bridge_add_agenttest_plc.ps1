...
if ($ProjectName -like '<*>' -or $TypeIdentifier -like '<*>' -or $DeviceName -like '<*>' -or $DeviceItemName -like '<*>') {
    throw "Replace -ProjectName, -TypeIdentifier, -DeviceName, and -DeviceItemName with real values."
}

$deviceReference = "$ProjectName/$DeviceName"
$inputText = @"
ADDDEVICE|$ProjectName|$TypeIdentifier|$DeviceName|$DeviceItemName
GETDEVICES|$ProjectName
GETDEVICEITEMS|$deviceReference
EXIT
"@
