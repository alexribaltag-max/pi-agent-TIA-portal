param(
    ...
    [Parameter(Mandatory = $false)]
    [string]$PlcDeviceReference = '<project-name>/<device-name>'
)

if ($PlcDeviceReference -like '<*>') {
    throw "Replace -PlcDeviceReference with a real PLC device reference."
}
...
function Read-UntilResponse {
    ...
    while ([DateTime]::UtcNow -lt $deadline) {
        ...
        $line = Read-LineWithTimeout -Reader $Process.StandardOutput -TimeoutMs $remaining
        if ($null -eq $line) {
            throw 'Process ended unexpectedly while waiting for response.'
        }
...
try {
    ...
    $null = Send-Command -Process $process -Command "GETPLCTAGTABLES|$PlcDeviceReference" -TimeoutMs 60000
